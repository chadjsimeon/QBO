...
export const managementReports = pgTable(
  "management_reports",
  {
    id: text("id")
      .primaryKey()
  ...
  (t) => [index("mgmt_reports_org_idx").on(t.organizationId)],
);

export const managementReportSections = pgTable(
  "management_report_sections",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
    managementReportId: text("management_report_id")
      .notNull()
      .references(() => managementReports.id, { onDelete: "cascade" }),
    reportType: managementReportTypeEnum("report_type").notNull(),
    sortOrder: integer("sort_order").default(0).notNull(),
  },
  (t) => [index("mgmt_report_sections_report_idx").on(t.managementReportId)],
);
