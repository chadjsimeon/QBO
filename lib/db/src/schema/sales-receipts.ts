...
export const salesReceipts = pgTable("sales_receipts", {
  ...
  depositAccountId: text("deposit_account_id").notNull().references(() => accounts.id),
  ...
  index("sales_receipts_org_idx").on(t.organizationId),
]);

export const salesReceiptLineItems = pgTable("sales_receipt_line_items", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  salesReceiptId: text("sales_receipt_id").notNull().references(() => salesReceipts.id, { onDelete: "cascade" }),
  ...
  index("sales_receipt_lines_idx").on(t.salesReceiptId),
]);
