...
export const vendorCreditLineItems = pgTable("vendor_credit_line_items", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  vendorCreditId: text("vendor_credit_id").notNull().references(() => vendorCredits.id, { onDelete: "cascade" }),
