...
if (!file) {
  const dumps = existsSync(backupsDir) ? readdirSync(backupsDir).filter((f) => f.endsWith(".dump")).sort() : [];
  if (!dumps.length) { console.error("No .dump files found in backups/. Run `pnpm db:backup` first."); process.exit(1); }
...
docker(["exec", CONTAINER, "psql", "-U", "qbo", "-d", "postgres", "-c", `CREATE DATABASE ${target}`], { stdio: "ignore" });
...
const res = spawnSync(
  "docker",
  ["exec", "-i", CONTAINER, "pg_restore", "--clean", "--if-exists", "--no-owner", "-U", "qbo", "-d", target],
  { stdio: [openSync(file, "r"), "inherit", "inherit"] },
);
if (res.status === 0) console.log(`✓ Restored into ${target}`);
else { console.error(`✗ pg_restore exited ${res.status}`); process.exit(res.status ?? 1); }
