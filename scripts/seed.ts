...
async function main() {
    ...
    .values([
      {
        ...
        phone: "555-0101",
        billingAddress: "1 Globex Way, Springfield, IL",
      },
      {
        organizationId: org.id,
        name: "Initech LLC",
  ...
  await db.insert(schema.invoiceLineItems).values({
    invoiceId: inv1.id,
    description: "Software consulting – Q2",
    quantity: 50,
  ...
  const [bill1] = await db
    ...
    .values({
      organizationId: org.id,
      vendorId: vend1.id,
      number: "BILL-AWS-0624",
      status: "OPEN",
      issueDate: d(-5),
      dueDate: d(25),
      subtotalCents: 328400,
      taxCents: 0,
      totalCents: 328400,
      balanceCents: 328400,
    })
  ...
  const [pmt1] = await db
    ...
    .values({
      ...
      memo: "Partial payment on INV-002",
    })
    .returning();
