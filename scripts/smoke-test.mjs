...
check(
  ...
  r.status === 400 && (await r.json()).error === "Malformed JSON body",
);
...
r = await api("POST", "/journal-entries", {
  date: "2026-06-01",
  lines: [
    { accountId: checking.id, debitCents: 500 },
    { accountId: income.id, creditCents: 300 },
  ],
});
...
await api("POST", `/bank-accounts/${ba.id}/transactions`, {
  csv: "2026-06-05,ACME PAYMENT,100.00",
});
const txns = (await api("GET", `/bank-accounts/${ba.id}/transactions?status=FOR_REVIEW`)).data;
check("txn imported", txns.length === 1, txns);

r = await api("POST", `/bank-transactions/${txns[0].id}/match`, {
  kind: "invoice",
  documentId: invId,
});
...
r = await api("GET", `/invoices/${invId}`);
check(
  "balance restored after undo",
  r.data.status === "SENT" && r.data.balanceCents === 10000,
  r.data,
);
