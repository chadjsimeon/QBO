...
router.post("/payments", validateBody(CreatePaymentBody), async (req, res) => {
  const orgId = req.session.organizationId!;
  const { direction, customerId, vendorId, amountCents, date, method, memo, allocations } =
    req.body;

  if (!["RECEIVED", "SENT"].includes(direction)) {
    res.status(400).json({ error: "direction must be RECEIVED or SENT" });
    return;
  }
  if (!amountCents || !date) {
...
router.get("/payments/open-invoices/:customerId", async (req, res) => {
  ...
  const open = rows.filter(
    (i) => ["SENT", "PARTIAL", "OVERDUE"].includes(i.status) && i.balanceCents > 0,
  );
  res.json(
    open.map((i) => ({
      ...i,
...
router.get("/payments/open-bills/:vendorId", async (req, res) => {
  ...
  const open = rows.filter(
    (b) => ["OPEN", "PARTIAL", "OVERDUE"].includes(b.status) && b.balanceCents > 0,
  );
  res.json(
    open.map((b) => ({
      ...b,
