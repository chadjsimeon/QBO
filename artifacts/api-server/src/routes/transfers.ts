...
router.post("/transfers", async (req, res) => {
  ...
  const { date, fromAccountId, toAccountId, amountCents, memo } = req.body as {
    date?: string; fromAccountId?: string; toAccountId?: string; amountCents?: number; memo?: string;
  };
  ...
  const accts = await db.select({ id: accounts.id }).from(accounts)
    .where(and(eq(accounts.organizationId, orgId), inArray(accounts.id, [fromAccountId, toAccountId])));
