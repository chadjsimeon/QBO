...
router.get("/bank-accounts", async (req, res) => {
  ...
  const rows = await db
    .select({ ba: bankAccounts, accountName: accounts.name })
    .from(bankAccounts)
  ...
  const counts = await db
    ...
    .where(
      and(eq(bankTransactions.organizationId, orgId), eq(bankTransactions.status, "FOR_REVIEW")),
    )
...
router.post("/bank-accounts/ensure", async (req, res) => {
  ...
  if (existing) {
    res.json(serializeBankAccount(existing.ba, existing.accountName));
    return;
  }
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        validStatus
          ? eq(bankTransactions.status, status as "FOR_REVIEW" | "CATEGORIZED" | "EXCLUDED")
          : undefined,
      ),
    )
...
router.post(
  ...
  validateBody(ImportBankTransactionsBody),
  async (req, res) => {
    const orgId = req.session.organizationId!;
    ...
    for (const raw of rows) {
      ...
      const [dateStr, desc, amountStr] = [
        cells[0],
        cells.slice(1, -1).join(",") || cells[1],
        cells[cells.length - 1],
      ];
      const date = new Date(dateStr.trim());
      const amountCents = Math.round(parseFloat(amountStr.trim()) * 100);
      if (isNaN(date.getTime()) || isNaN(amountCents)) {
        skipped++;
        continue;
      }
...
router.post(
  ...
  validateBody(CategorizeBankTransactionBody),
  async (req, res) => {
    const orgId = req.session.organizationId!;
    ...
    const [txn] = await db
      ...
      .from(bankTransactions)
      .where(
        and(eq(bankTransactions.id, req.params.id), eq(bankTransactions.organizationId, orgId)),
      );
    if (!txn) {
      res.status(404).json({ error: "Not found" });
      return;
    ...
    if (txn.status === "CATEGORIZED") {
      res.status(409).json({ error: "Already categorized" });
      return;
    }
...
router.post("/bank-transactions/bulk-categorize", async (req, res) => {
  const orgId = req.session.organizationId!;
  const { ids, accountId, memo } = req.body as {
    ids?: string[];
    accountId?: string;
    memo?: string;
  };
...
router.post("/bank-transactions/:id/exclude", async (req, res) => {
  ...
  if (txn.status === "CATEGORIZED") {
    res.status(409).json({ error: "Undo the categorization before excluding" });
    return;
  }
...
router.post("/bank-transactions/:id/match", async (req, res) => {
  ...
  if (kind !== "invoice" && kind !== "bill") {
    res.status(400).json({ error: "kind must be invoice or bill" });
    return;
  }
  if (!documentId) {
  ...
  if (!systemAccount) {
    res.status(400).json({ error: `No ${role} account configured` });
    return;
  }
  ...
  if (currentBalance <= 0) {
    res.status(409).json({ error: `This ${noun} is already paid` });
    return;
  }
  if (magnitude > currentBalance) {
    res.status(409).json({ error: `Amount exceeds the ${noun}'s open balance` });
    return;
  }
...
function bestMatch(
  ...
  let best: { kind: string; id: string; number: string; label: string; confidence: number } | null =
    null;
  ...
  for (const c of candidates) {
    ...
    if (!best || conf > best.confidence) {
      best = {
        ...
        id: c.id,
        number: c.number,
        label: c.name ? `${c.number} · ${c.name}` : c.number,
...
router.post("/bank-accounts/:id/import/analyze", async (req, res) => {
  ...
  const incoming = (req.body?.transactions ?? []) as Array<{
    date: string;
    amountCents: number;
    description: string;
  }>;
  ...
  for (const e of existing)
    existKeys.set(
      `${dayKey(e.date)}|${e.amountCents}|${(e.descriptionRaw || "").trim().toLowerCase()}`,
      e.id,
    );

  const openInvoices = await db
    .select({ inv: invoices, name: customers.name })
    .from(invoices)
  ...
  const openBills = await db
    .select({ bill: bills, name: vendors.name })
    .from(bills)
    .leftJoin(vendors, eq(bills.vendorId, vendors.id))
    .where(
      and(eq(bills.organizationId, orgId), inArray(bills.status, ["OPEN", "PARTIAL", "OVERDUE"])),
    );

  const invCands: MatchCandidate[] = openInvoices.map((x) => ({
    id: x.inv.id,
    number: x.inv.number,
    balanceCents: x.inv.balanceCents,
    date: x.inv.dueDate,
    name: x.name,
  }));
  const billCands: MatchCandidate[] = openBills.map((x) => ({
    id: x.bill.id,
    number: x.bill.number,
    balanceCents: x.bill.balanceCents,
    date: x.bill.dueDate,
    name: x.name,
  }));
  ...
  const results = incoming.map((t, index) => {
    ...
    if (t.amountCents > 0) match = bestMatch(d, t.amountCents, t.description, invCands, "invoice");
      ...
      match = bestMatch(d, Math.abs(t.amountCents), t.description, billCands, "bill");
...
router.post("/bank-accounts/:id/import/commit", async (req, res) => {
  ...
  for (let i = 0; i < transactions.length; i++) {
    ...
    const d = new Date(t.date);
    if (isNaN(d.getTime())) {
      failures.push({ index: i, reason: "Invalid date" });
      continue;
    }
    if (typeof t.amountCents !== "number" || isNaN(t.amountCents)) {
      failures.push({ index: i, reason: "Invalid amount" });
      continue;
    }
    if (!t.description) {
      failures.push({ index: i, reason: "Missing description" });
      continue;
    }
    try {
      const [row] = await db
        ...
        .values({
          ...
          bankAccountId,
          date: d,
          descriptionRaw: String(t.description),
