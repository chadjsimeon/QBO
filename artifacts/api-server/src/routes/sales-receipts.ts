...
import { db, salesReceipts, salesReceiptLineItems, customers, accounts, journalEntries, journalLines } from "@workspace/db";
...
function serialize(r: typeof salesReceipts.$inferSelect, customerName?: string | null, depositAccountName?: string | null) {
  return {
    ...r,
...
router.get("/sales-receipts", async (req, res) => {
  ...
  const rows = await db.select({ r: salesReceipts, customerName: customers.name, acctName: accounts.name })
    .from(salesReceipts)
...
router.get("/sales-receipts/:id", async (req, res) => {
  const orgId = req.session.organizationId!;
  const [row] = await db.select({ r: salesReceipts, customerName: customers.name, acctName: accounts.name })
    .from(salesReceipts)
    .leftJoin(customers, eq(salesReceipts.customerId, customers.id))
    .leftJoin(accounts, eq(salesReceipts.depositAccountId, accounts.id))
    .where(and(eq(salesReceipts.id, req.params.id), eq(salesReceipts.organizationId, orgId)));
  if (!row) { res.status(404).json({ error: "Not found" }); return; }
  const lines = await db.select().from(salesReceiptLineItems)
...
router.post("/sales-receipts", async (req, res) => {
  ...
  const incomeLegs: Array<{ accountId: string; cents: number }> = computed.map(l => ({ accountId: l.accountId, cents: l.amountCents }));
  if (taxCents > 0) incomeLegs.push({ accountId: taxAccountId ?? computed[0].accountId, cents: taxCents });
  ...
  if (isRefund) {
    ...
    for (const leg of incomeLegs) entryLines.push({ accountId: leg.accountId, debitCents: leg.cents, creditCents: 0 });
...
router.post("/sales-receipts/:id/void", async (req, res) => {
  const orgId = req.session.organizationId!;
  const [sr] = await db.select().from(salesReceipts)
    .where(and(eq(salesReceipts.id, req.params.id), eq(salesReceipts.organizationId, orgId)));
