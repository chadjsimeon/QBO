...
router.get("/statements/customer/:id", async (req, res) => {
  ...
  const invRows = await db
    .select()
    .from(invoices)
    .where(and(eq(invoices.organizationId, orgId), eq(invoices.customerId, customer.id)));
  ...
  const cnRows = await db
    .select()
    .from(creditNotes)
    .where(and(eq(creditNotes.organizationId, orgId), eq(creditNotes.customerId, customer.id)));
  ...
  for (const p of payRows)
    all.push({ date: p.date, type: "Payment", ref: p.method, charge: 0, credit: p.amountCents });
  for (const c of cnRows)
    all.push({ date: c.date, type: "Credit note", ref: c.number, charge: 0, credit: c.totalCents });
