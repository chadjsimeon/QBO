...
const serialize = (r: typeof accounts.$inferSelect) => ({ ...r, createdAt: r.createdAt.toISOString() });
...
router.patch("/accounts/:id", async (req, res) => {
  ...
  if (type !== undefined) {
    ...
    if (existing.systemRole && type !== existing.type) {
      res.status(400).json({ error: "Cannot change the type of a system account" }); return;
    }
...
router.delete("/accounts/:id", async (req, res) => {
  ...
  if (existing.systemRole) {
    res.status(400).json({ error: "System accounts cannot be deleted. Deactivate it instead." }); return;
  }

  // Block deletion of accounts that already carry ledger activity to preserve double-entry history.
  const [used] = await db.select({ id: journalLines.id }).from(journalLines)
    .where(eq(journalLines.accountId, req.params.id)).limit(1);
  if (used) {
    res.status(409).json({ error: "This account has transactions and cannot be deleted. Deactivate it instead." }); return;
  }
