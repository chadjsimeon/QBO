...
router.get("/credit-notes", async (req, res) => {
  ...
  const rows = await db
    .select({ c: creditNotes, customerName: customers.name })
    .from(creditNotes)
    .leftJoin(customers, eq(creditNotes.customerId, customers.id))
    .where(eq(creditNotes.organizationId, orgId))
...
router.get("/credit-notes/:id", async (req, res) => {
  ...
  const lines = await db
    ...
    .where(eq(creditNoteLineItems.creditNoteId, req.params.id))
    .orderBy(creditNoteLineItems.sortOrder);
  const apps = await db
    .select({ a: creditNoteApplications, number: invoices.number })
  ...
  res.json({
    ...serialize(row.c, row.customerName),
    lineItems: lines,
    applications: apps.map((x) => ({ ...x.a, invoiceNumber: x.number })),
  });
});
...
router.post("/credit-notes", async (req, res) => {
  ...
  if (!customerId || !number || !date || !lines?.length) {
  ...
  if (taxCents > 0) {
    try {
      ...
      entryLines.push({ accountId: tax.id, debitCents: taxCents, creditCents: 0 });
    } catch {
      entryLines[0].debitCents += taxCents;
    }
  }
  ...
  const cn = await db.transaction(async (tx) => {
    ...
    await tx
      ...
      .where(eq(creditNotes.id, row.id));
...
router.post("/credit-notes/:id/apply", async (req, res) => {
  ...
  const apps = (applications ?? [])
    .map((a) => ({ invoiceId: a.invoiceId, amountCents: Math.round(Number(a.amountCents) || 0) }))
    .filter((a) => a.invoiceId && a.amountCents > 0);
  const total = apps.reduce((s, a) => s + a.amountCents, 0);
  if (total <= 0) {
    res.status(400).json({ error: "Enter an amount to apply." });
    return;
  }
  if (total > cn.balanceCents) {
  ...
  const invs = await db
    .select()
    .from(invoices)
    .where(and(eq(invoices.organizationId, orgId), inArray(invoices.id, invIds)));
  const invMap = new Map(invs.map((i) => [i.id, i]));
  for (const a of apps) {
    const inv = invMap.get(a.invoiceId);
    if (!inv || inv.customerId !== cn.customerId) {
      res.status(400).json({ error: "Invoice not found for this customer." });
      return;
    }
    if (a.amountCents > inv.balanceCents) {
      res.status(400).json({ error: `Amount exceeds balance on ${inv.number}.` });
      return;
    }
  }
  ...
  await db.transaction(async (tx) => {
    for (const a of apps) {
      ...
      await tx
        ...
        .values({ creditNoteId: cn.id, invoiceId: a.invoiceId, amountCents: a.amountCents });
    }
    await tx
      .update(creditNotes)
      .set({ balanceCents: cn.balanceCents - total })
...
router.post("/credit-notes/:id/void", async (req, res) => {
  ...
  if (cn.balanceCents !== cn.totalCents) {
    res.status(400).json({
      error: "Cannot void a credit note that has been applied. Remove applications first.",
    });
    return;
  }
  ...
  await db.transaction(async (tx) => {
    const [je] = await tx
      .select()
      .from(journalEntries)
      .where(and(eq(journalEntries.organizationId, orgId), eq(journalEntries.sourceId, cn.id)))
      .limit(1);
    if (je) {
      ...
      await postEntry(
        {
          organizationId: orgId,
          date: new Date(),
          memo: `Void credit note ${cn.number}`,
