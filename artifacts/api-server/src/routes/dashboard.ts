...
router.get("/dashboard/summary", async (req, res) => {
  ...
  const [arResult, apResult, cashAccounts, recentEntries] = await Promise.all([
    db
      .select({ total: sql<number>`SUM(balance_cents)`.as("total") })
      .from(invoices)
      .where(
        and(
...
router.get("/dashboard/home", async (req, res) => {
  ...
  const [accts, lastMonthNet, priorMonthNet, allTimeNet, cashFlowRows, recentFeed] =
    await Promise.all([
      ...
      db
        .select({
          month: sql<string>`TO_CHAR(DATE_TRUNC('month', ${journalEntries.date}), 'Mon')`.as(
            "month",
          ),
          monthNum: sql<string>`TO_CHAR(DATE_TRUNC('month', ${journalEntries.date}), 'YYYY-MM')`.as(
            "month_num",
          ),
  ...
  const changePercent =
    priorNet !== 0 ? Math.round(((lastNet - priorNet) / Math.abs(priorNet)) * 100) : 0;

  const priorExpenses = expenseTotal(priorMonthNet);
  const expenseChangePercent =
    priorExpenses !== 0
      ? Math.round(((lastExpenses - priorExpenses) / Math.abs(priorExpenses)) * 100)
      : 0;
...
router.get("/search", async (req, res) => {
  ...
  if (!q) {
    res.json({ customers: [], vendors: [], invoices: [], bills: [] });
    return;
  }
