...
router.get("/expenses", async (req, res) => {
  ...
  const rows = await db
    .select({ e: expenses, vendorName: vendors.name, acctName: accounts.name })
    .from(expenses)
...
router.post("/expenses", async (req, res) => {
  const orgId = req.session.organizationId!;
  const { vendorId, paymentAccountId, number, refNumber, method, date, memo, lines, isCredit } =
    req.body;
  ...
  if (isCredit) {
    ...
    if (taxCents > 0)
      entryLines.push({
        accountId: taxAccountId ?? computed[0].accountId,
        debitCents: 0,
        creditCents: taxCents,
      });
  } else {
    for (const l of computed)
      entryLines.push({ accountId: l.accountId, debitCents: l.amountCents, creditCents: 0 });
    if (taxCents > 0)
      entryLines.push({
        accountId: taxAccountId ?? computed[0].accountId,
        debitCents: taxCents,
  ...
  const exp = await db.transaction(async (tx) => {
    ...
    await tx.insert(expenseLineItems).values(
      computed.map((l, i) => ({
        expenseId: row.id,
  ...
  const [je] = await db
    ...
    .where(
      and(
        ...
        eq(journalEntries.sourceId, exp.id),
      ),
    )
    .limit(1);
  if (je) {
