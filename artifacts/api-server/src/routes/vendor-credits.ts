...
router.get("/vendor-credits", async (req, res) => {
  ...
  const rows = await db
    .select({ c: vendorCredits, vendorName: vendors.name })
    .from(vendorCredits)
    .leftJoin(vendors, eq(vendorCredits.vendorId, vendors.id))
    .where(eq(vendorCredits.organizationId, orgId))
    .orderBy(desc(vendorCredits.date));
...
router.get("/vendor-credits/:id", async (req, res) => {
  ...
  const lines = await db
    .select()
    .from(vendorCreditLineItems)
    .where(eq(vendorCreditLineItems.vendorCreditId, req.params.id))
    .orderBy(vendorCreditLineItems.sortOrder);
  const apps = await db
    .select({ a: vendorCreditApplications, number: bills.number })
    .from(vendorCreditApplications)
    .leftJoin(bills, eq(vendorCreditApplications.billId, bills.id))
    .where(eq(vendorCreditApplications.vendorCreditId, req.params.id));
  res.json({
    ...serialize(row.c, row.vendorName),
    lineItems: lines,
    applications: apps.map((x) => ({ ...x.a, billNumber: x.number })),
  });
});
...
router.post("/vendor-credits", async (req, res) => {
  ...
  if (!vendorId || !number || !date || !lines?.length) {
  ...
  for (const l of computed)
    entryLines.push({ accountId: l.accountId, debitCents: 0, creditCents: l.amountCents });
  if (taxCents > 0) {
    try {
      const tax = await findSystemAccount(orgId, "SALES_TAX_PAYABLE");
      entryLines.push({ accountId: tax.id, debitCents: 0, creditCents: taxCents });
    } catch {
      entryLines[1].creditCents += taxCents;
    }
  }
  ...
  const vc = await db.transaction(async (tx) => {
    ...
    await tx.insert(vendorCreditLineItems).values(
      computed.map((l, i) => ({
    ...
    const entry = await postEntry(
      {
        organizationId: orgId,
        date: new Date(date),
        memo: memo || `Supplier credit ${number}`,
...
router.post("/vendor-credits/:id/apply", async (req, res) => {
  const orgId = req.session.organizationId!;
  const { applications } = req.body as {
  ...
  const apps = (applications ?? [])
    .map((a) => ({ billId: a.billId, amountCents: Math.round(Number(a.amountCents) || 0) }))
    .filter((a) => a.billId && a.amountCents > 0);
  const total = apps.reduce((s, a) => s + a.amountCents, 0);
  if (total <= 0) {
    res.status(400).json({ error: "Enter an amount to apply." });
    return;
  }
  if (total > vc.balanceCents) {
    res.status(400).json({ error: "Applied amount exceeds the available credit." });
    return;
  }
  ...
  for (const a of apps) {
    ...
    if (!bill || bill.vendorId !== vc.vendorId) {
      res.status(400).json({ error: "Bill not found for this vendor." });
      return;
    }
    if (a.amountCents > bill.balanceCents) {
      res.status(400).json({ error: `Amount exceeds balance on ${bill.number}.` });
      return;
    }
  }
  ...
  await db.transaction(async (tx) => {
    for (const a of apps) {
      ...
      await tx
        .update(bills)
        .set({ balanceCents: newBal, status: newBal <= 0 ? "PAID" : "PARTIAL" })
        .where(eq(bills.id, a.billId));
      await tx
        .insert(vendorCreditApplications)
        .values({ vendorCreditId: vc.id, billId: a.billId, amountCents: a.amountCents });
    }
    await tx
      .update(vendorCredits)
      .set({ balanceCents: vc.balanceCents - total })
...
router.post("/vendor-credits/:id/void", async (req, res) => {
  ...
  if (vc.balanceCents !== vc.totalCents) {
    res.status(400).json({ error: "Cannot void a credit that has been applied." });
    return;
  }
  ...
  await db.transaction(async (tx) => {
    const [je] = await tx
      .select()
      .from(journalEntries)
      .where(and(eq(journalEntries.organizationId, orgId), eq(journalEntries.sourceId, vc.id)))
      .limit(1);
    if (je) {
      ...
      await postEntry(
        {
          organizationId: orgId,
          date: new Date(),
          memo: `Void supplier credit ${vc.number}`,
