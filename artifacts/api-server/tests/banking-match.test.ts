...
async function createIssuedInvoice(org: TestOrg, number: string, cents: number) {
  ...
  const cust = (await org.agent.post("/api/customers").send({ name: `Cust ${number}` }).expect(201)).body;
  const inv = (await org.agent.post("/api/invoices").send({
    contactId: cust.id,
    number,
...
async function importTxn(org: TestOrg, bankAccountId: string, csvLine: string) {
  await org.agent.post(`/api/bank-accounts/${bankAccountId}/transactions`).send({ csv: csvLine }).expect(200);
  const txns = (await org.agent.get(`/api/bank-accounts/${bankAccountId}/transactions?status=FOR_REVIEW`).expect(200)).body;
...
describe("bank transaction match", () => {
  ...
  beforeAll(async () => {
    ...
    bankAccountId = (await org.agent.post("/api/bank-accounts/ensure").send({ accountId: checking }).expect(200)).body.id;
  });
  ...
  it("pays an invoice down to PAID on an exact match", async () => {
    ...
    const matched = (await org.agent.post(`/api/bank-transactions/${txn.id}/match`)
      .send({ kind: "invoice", documentId: inv.id }).expect(200)).body;
  ...
  it("marks a partial match PARTIAL", async () => {
    ...
    await org.agent.post(`/api/bank-transactions/${txn.id}/match`)
      .send({ kind: "invoice", documentId: inv.id }).expect(200);
  ...
  it("rejects matching against an already-paid invoice with 409 and posts nothing", async () => {
    ...
    await org.agent.post(`/api/bank-transactions/${t1.id}/match`)
      .send({ kind: "invoice", documentId: inv.id }).expect(200);

    const t2 = await importTxn(org, bankAccountId, "2026-06-08,PAY TWICE,50.00");
    await org.agent.post(`/api/bank-transactions/${t2.id}/match`)
      .send({ kind: "invoice", documentId: inv.id }).expect(409);
  ...
  it("categorize posts to the ledger and undo reverses it", async () => {
    ...
    const cat = (await org.agent.post(`/api/bank-transactions/${txn.id}/categorize`)
      .send({ accountId: fees }).expect(200)).body;
    ...
    const list = (await org.agent.get(`/api/bank-accounts/${bankAccountId}/transactions?status=FOR_REVIEW`).expect(200)).body;
