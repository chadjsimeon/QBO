...
export default function BankImportPage() {
  ...
  const { data: unlinkedAccounts = [] } = useQuery({
    ...
    queryFn: () => apiFetch<CoaBankAccount[]>("/bank-accounts/unlinked"),
  });
  ...
  const dropdownOptions = useMemo(
    () => [
      ...bankAccounts.map((b) => ({
        value: `linked:${b.id}`,
        label: `${b.institutionName}${b.accountMask ? ` ···${b.accountMask}` : ""}${b.accountName ? ` — ${b.accountName}` : ""}`,
      })),
    ...
    Papa.parse<Record<string, string>>(file, {
      ...
      complete: (res) => {
        ...
        if (flds.length === 0 || res.data.length === 0) {
          toast({
            title: "Empty file",
            description: "No header row or rows detected.",
            variant: "destructive",
          });
          return;
        }
  ...
  const analyze = useMutation({
    mutationFn: async () => {
      ...
      if (!resolvedId && selectedOption.startsWith("coa:")) {
        ...
        const ensured = await apiFetch<BankAccount>("/bank-accounts/ensure", {
          method: "POST",
          body: JSON.stringify({ accountId: coaId }),
        });
        resolvedId = ensured.id;
        setBankAccountId(resolvedId);
      }
      return apiFetch<{ results: AnalyzeRow[]; duplicateCount: number; matchCount: number }>(
        `/bank-accounts/${resolvedId}/import/analyze`,
        {
          method: "POST",
          body: JSON.stringify({
      ...
      setReview(
        validRows.map((r, i) => {
          ...
          return {
            ...
            match: a?.match ?? null,
            acceptMatch: (a?.match?.confidence ?? 0) >= 0.8,
          };
        }),
      );
      setStep(3);
  ...
  const commit = useMutation({
    mutationFn: () => {
      ...
      return apiFetch<typeof result>(`/bank-accounts/${bankAccountId}/import/commit`, {
        method: "POST",
        body: JSON.stringify({ fileName, skipDuplicates, transactions: txns }),
      });
    },
    onSuccess: (data) => {
      setProgress(100);
  ...
  return (
    <>
      ...
      {step === 1 && (
        <Card>
          <CardHeader>
            <CardTitle>Upload a CSV file with your bank transactions</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="max-w-md space-y-2">
              ...
              <Select
                ...
                onChange={(e) => {
                  ...
                  setSelectedOption(v);
                  setBankAccountId(v.startsWith("linked:") ? v.slice(7) : "");
                }}
              >
              ...
              {dropdownOptions.length === 0 && (
                <p className="text-xs text-muted-foreground">
                  No bank accounts yet. Create one in Chart of Accounts first.
                </p>
              )}
            </div>
            ...
            >
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                CSV only · up to 25MB · first row should be column headers
              </p>
            </div>
          </CardContent>
        </Card>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {[...REQUIRED_FIELDS, ...OPTIONAL_FIELDS].map((field) => {
                  ...
                  const sample = mapping[field]
                    ? (rows.find((r) => r[mapping[field]])?.[mapping[field]] ?? "")
                    : "";
                  return (
          ...
          <div className="flex justify-between">
            ...
            >
              {analyze.isPending ? (
                ...
                <>
                  Validate &amp; check duplicates <ArrowRight className="h-4 w-4 ml-1" />
                </>
              )}
            </Button>
          </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <StatCard
              ...
              value={String(review.filter((r) => r.isDuplicate).length)}
              tone={review.some((r) => r.isDuplicate) ? "warn" : undefined}
            />
          </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-1 text-sm max-h-48 overflow-auto">
                ...
                <p className="pt-2 text-xs text-muted-foreground">
                  Invalid rows are excluded from the import. Go back to fix the mapping/date format,
          ...
          <Card>
            <CardContent className="flex items-center justify-between pt-6">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Matched on Date + Amount + Description against existing transactions.
                </p>
              </div>
          ...
          <div className="flex justify-between">
            ...
            <Button disabled={validRows.length === 0} onClick={() => setStep(4)}>
              Continue to matching <ArrowRight className="h-4 w-4 ml-1" />
            </Button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                {review.filter((r) => r.match).length} suggestion(s) found. Deposits match open
                invoices; payments match open bills.
              </p>
            </CardHeader>
            <CardContent className="p-0">
              {review.filter((r) => r.match).length === 0 ? (
                <p className="p-6 text-sm text-muted-foreground">
                  No suggested matches. You can categorize these later from the Banking review
                  screen.
                </p>
              ) : (
                <Table>
                      ...
                      (r, i) =>
                        r.match && (
                          <TableRow key={i}>
                            ...
                            <TableCell>
                              {r.match.kind === "invoice" ? "Invoice " : "Bill "}
                              {r.match.label}
                            </TableCell>
                            <TableCell>
                              <Badge variant={r.match.confidence >= 0.8 ? "default" : "secondary"}>
                            ...
                            <TableCell className="text-right">
                              <input
                                type="checkbox"
                                className="h-4 w-4"
                                checked={r.acceptMatch}
                                onChange={(e) =>
                                  setReview((rs) =>
                                    rs.map((x, idx) =>
                                      idx === i ? { ...x, acceptMatch: e.target.checked } : x,
                                    ),
                                  )
                                }
                              />
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <StatCard
              label="Date range"
              value={
                summary.from ? `${formatDate(summary.from)} – ${formatDate(summary.to!)}` : "—"
              }
              small
            />
          ...
          <Card className="overflow-hidden">
            <CardHeader>
              <CardTitle className="text-base">All transactions to import</CardTitle>
            </CardHeader>
            <CardContent className="p-0 overflow-x-auto">
              <Table>
                <TableHeader>
                      ...
                      <TableRow key={i} className={r.include ? "" : "opacity-40"}>
                        {isEditing ? (
                          <>
                            <TableCell>
                              <Input
                                type="date"
                                className="h-8 w-36"
                                value={r.date ? toDateInput(r.date) : ""}
                                onChange={(e) =>
                                  setReview((rs) =>
                                    rs.map((x, idx) =>
                                      idx === i
                              ...
                              <Input
                                ...
                                onChange={(e) => {
                                  ...
                                  setReview((rs) =>
                                    rs.map((x, idx) =>
                                      idx === i
                                        ? { ...x, amountCents: isNaN(c) ? x.amountCents : c }
                                        : x,
                                    ),
                                  );
                                }}
                            ...
                            <TableCell>
                              <Input
                                ...
                                onChange={(e) =>
                                  setReview((rs) =>
                                    rs.map((x, idx) =>
                                      idx === i ? { ...x, referenceNumber: e.target.value } : x,
                                    ),
                                  )
                                }
                              />
                            ...
                            <TableCell>
                              <Input
                                ...
                                onChange={(e) =>
                                  setReview((rs) =>
                                    rs.map((x, idx) =>
                                      idx === i ? { ...x, payeeName: e.target.value } : x,
                                    ),
                                  )
                                }
                              />
                          ...
                          <>
                            ...
                            <TableCell>
                              <Badge variant={deposit ? "default" : "secondary"}>
                                {deposit ? "Deposit" : "Payment"}
                              </Badge>
                            </TableCell>
                            <TableCell
                              className={`text-right tabular-nums ${deposit ? "text-green-600" : ""}`}
                            >
                              {deposit ? "+" : "−"}
                              {formatCents(Math.abs(r.amountCents ?? 0))}
                            </TableCell>
                            <TableCell className="max-w-[16rem] truncate">
                              {r.description}
                              {r.isDuplicate && (
                            ...
                            <TableCell className="text-muted-foreground text-xs">
                              {r.acceptMatch && r.match ? r.match.label : "—"}
                            </TableCell>
                            <TableCell className="text-right">
                              <Button
                                size="icon"
          ...
          <Card className="border-amber-300 bg-amber-50/50">
            <CardContent className="space-y-3 pt-6">
              <p className="flex items-center gap-2 text-sm text-amber-800">
                <AlertTriangle className="h-4 w-4" /> These transactions will be imported to your
                account.
              </p>
          ...
          <div className="flex items-center justify-between">
            ...
            <Button
              disabled={!confirmed || summary.count === 0}
              onClick={() => {
                setStep(6);
                commit.mutate();
              }}
        ...
        <div className="space-y-6">
          <Card className="border-green-300 bg-green-50/50">
            <CardContent className="flex items-center gap-3 pt-6">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {result.imported} imported · {result.skipped} duplicates skipped · {result.failed}{" "}
                  failed
                </p>
              </div>
            </CardContent>
          ...
          {result.failed > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="text-base flex items-center gap-2">
                  <AlertTriangle className="h-4 w-4 text-destructive" /> {result.failed} failed to
                  import
                </CardTitle>
              </CardHeader>
          ...
          {result.transactions.length > 0 && (
            <Card className="overflow-hidden">
              <CardHeader>
                <CardTitle className="text-base">Imported transactions (Pending review)</CardTitle>
              </CardHeader>
              <CardContent className="p-0 overflow-x-auto">
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {result.transactions.map((t: any) => (
                      <TableRow key={t.id}>
                        ...
                        <TableCell
                          className={`text-right tabular-nums ${t.amountCents > 0 ? "text-green-600" : ""}`}
                        >
                          {t.amountCents > 0 ? "+" : "−"}
          ...
          <div className="flex gap-3">
            <Button asChild>
              <Link href={bankAccountId ? `/banking/${bankAccountId}` : "/banking"}>
                Review &amp; categorize
              </Link>
            </Button>
            <Button
  ...
  return (
    <Card>
      <CardContent className="pt-5">
        ...
        <div className={`mt-1 font-bold tabular-nums ${small ? "text-sm" : "text-xl"} ${color}`}>
          {value}
        </div>
      </CardContent>
    </Card>
