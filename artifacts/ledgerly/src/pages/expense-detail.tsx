      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold">
            {exp.number}
            {exp.refNumber ? ` · #${exp.refNumber}` : ""}
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            {exp.vendorName ?? "No payee"} · paid from {exp.paymentAccountName ?? "—"} ·{" "}
            {formatDate(exp.date)}
          </p>
        </div>
        <Button
          ...
          size="sm"
          onClick={() => {
            if (confirm("Void this expense? A reversing entry will be posted.")) voidIt.mutate();
          }}
        ...
        <CardContent className="p-0">
          ...
          <div className="flex justify-end gap-12 border-t px-4 py-3 text-sm">
            ...
            <div className="space-y-1 text-right tabular-nums">
              <div>{formatCents(exp.subtotalCents)}</div>
              <div>{formatCents(exp.taxCents)}</div>
