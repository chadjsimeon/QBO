    ...
    <>
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {ccMode ? "Pay a credit card balance from a bank account." : "Move money between two of your accounts."}
        </p>
      </div>

      <form onSubmit={e => { e.preventDefault(); if (valid) save.mutate(); }} className="space-y-6 max-w-2xl">
        <Card>
          <CardHeader><CardTitle>{ccMode ? "Payment" : "Transfer"}</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] gap-3 items-end">
              <div className="space-y-2">
                ...
                <Select value={fromAccountId} onChange={e => setFrom(e.target.value)} required>
                  ...
                  {fromOptions.map(a => <option key={a.id} value={a.id}>{a.code} · {a.name}</option>)}
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select value={toAccountId} onChange={e => setTo(e.target.value)} required>
                  <option value="">Select…</option>
                  {toOptions.map(a => <option key={a.id} value={a.id}>{a.code} · {a.name}</option>)}
                </Select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="amount" type="number" min={1} step={1} value={amountCents || ""}
                  onChange={e => setAmount(parseInt(e.target.value) || 0)} required />
                {amountCents > 0 && <p className="text-xs text-muted-foreground">{formatCents(amountCents)}</p>}
              </div>
              <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input id="memo" value={memo} onChange={e => setMemo(e.target.value)} placeholder="Optional note" />
            </div>
          </CardContent>
        </Card>

        <div className="flex justify-end gap-3 max-w-2xl">
          <Button type="button" variant="outline" asChild><Link href="/dashboard">Cancel</Link></Button>
