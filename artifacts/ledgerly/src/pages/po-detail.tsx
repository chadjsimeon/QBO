      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {po.vendorName ?? "—"} · {formatDate(po.issueDate)}
            {po.expectedDate ? ` · expected ${formatDate(po.expectedDate)}` : ""}
          </p>
        </div>
        <div className="flex gap-2">
          {!converted && po.status !== "SENT" && (
          ...
          {converted ? (
            <Button size="sm" asChild>
              <Link href={`/bills/${po.convertedBillId}`}>
                View bill <ArrowRight className="h-4 w-4 ml-1" />
              </Link>
            </Button>
          ) : (
        ...
        <CardContent className="p-0">
          ...
          <div className="flex justify-end gap-12 border-t px-4 py-3 text-sm">
            ...
            <div className="space-y-1 text-right tabular-nums">
              <div>{formatCents(po.subtotalCents)}</div>
              <div>{formatCents(po.taxCents)}</div>
              <div className="font-semibold">{formatCents(po.totalCents)}</div>
            </div>
          </div>
        </CardContent>
      </Card>
