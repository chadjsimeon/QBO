    ...
    <>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Customer credits you can apply against open invoices.
          </p>
        </div>
      ...
      {notes.length === 0 ? (
        <EmptyState
          ...
          description="Issue a customer credit and apply it to their invoices."
          action={
            <Link href="/credit-notes/new">
              <Button>
              ...
              {notes.map((n) => (
                <TableRow key={n.id} className="cursor-pointer">
                  <TableCell className="text-muted-foreground">
                    <Link href={`/credit-notes/${n.id}`} className="hover:underline text-primary">
                      {formatDate(n.date)}
                    </Link>
                  </TableCell>
