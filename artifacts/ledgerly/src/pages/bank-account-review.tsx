...
export default function BankAccountReviewPage() {
  ...
  function groupsFor(deposit: boolean): Array<[string, Account[]]> {
    ...
    return order
      .map(
        (t) => [TYPE_LABEL[t], categoryAccounts.filter((a) => a.type === t)] as [string, Account[]],
      )
  ...
  const categorize = useMutation({
    mutationFn: ({ txnId, accountId }: { txnId: string; accountId: string }) =>
      apiFetch(`/bank-transactions/${txnId}/categorize`, {
        method: "POST",
  ...
  const bulkCategorize = useMutation({
    mutationFn: ({ ids, accountId }: { ids: string[]; accountId: string }) =>
      apiFetch<{ categorized: number }>("/bank-transactions/bulk-categorize", {
        method: "POST",
    ...
    onSuccess: (data) => {
      ...
      setSelected(new Set());
      setBulkAccountId("");
      toast({ title: `${data.categorized} transactions categorized` });
    },
    onError: (e: Error) =>
      toast({ title: "Error", description: e.message, variant: "destructive" }),
  });
  ...
  const match = useMutation({
    mutationFn: ({
      ...
      apiFetch(`/bank-transactions/${txnId}/match`, {
        method: "POST",
        body: JSON.stringify({ kind, documentId }),
      }),
    onSuccess: () => {
      invalidate();
      toast({ title: "Matched and posted" });
    },
    onError: (e: Error) =>
      toast({ title: "Error", description: e.message, variant: "destructive" }),
  });
  ...
  const exclude = useMutation({
    mutationFn: (txnId: string) =>
      apiFetch(`/bank-transactions/${txnId}/exclude`, { method: "POST" }),
    onSuccess: () => {
      invalidate();
  ...
  const undo = useMutation({
    ...
    onSuccess: () => {
      invalidate();
      toast({ title: "Moved back to For Review" });
    },
    onError: (e: Error) =>
      toast({ title: "Error", description: e.message, variant: "destructive" }),
  });
  ...
  const toggleAll = () =>
    setSelected(allSelected ? new Set() : new Set(forReview.map((t) => t.id)));
    ...
    <>
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold">
          ...
          {account?.accountMask && (
            <span className="text-muted-foreground font-normal"> ···{account.accountMask}</span>
          )}
        </h1>
        {account?.accountName && (
          <p className="text-sm text-muted-foreground">GL account: {account.accountName}</p>
        )}
      </div>
        ...
        <Card className="mb-3">
          <CardContent className="flex flex-wrap items-center gap-3 py-3">
            ...
            <Button
              ...
              onClick={() =>
                bulkCategorize.mutate({ ids: [...selected], accountId: bulkAccountId })
              }
            >
      ...
      {rows.length === 0 ? (
        <EmptyState
          ...
          description={
            tab === "FOR_REVIEW"
              ? "Imported transactions awaiting categorization will appear here."
              : ""
          }
        />
      ) : (
            ...
            <Table>
              ...
              <TableBody>
                {rows.map((t) => {
                  ...
                  const matchKind = t.matchedInvoiceId
                    ? "invoice"
                    : t.matchedBillId
                      ? "bill"
                      : null;
                    ...
                    <TableRow key={t.id}>
                      ...
                      {tab === "CATEGORIZED" && (
                        <TableCell className="text-sm">
                          {t.matchType === "MATCHED" ? (
                            <Badge variant="muted">
                              Matched {t.matchedInvoiceId ? "invoice" : "bill"}
                            </Badge>
                          ) : (
                      ...
                      <TableCell className="text-right">
                        {tab === "FOR_REVIEW" && (
                          <div className="flex justify-end gap-1">
                            ...
                            <Button
                              size="sm"
                              disabled={!pick[t.id] || categorize.isPending}
                              onClick={() =>
                                categorize.mutate({ txnId: t.id, accountId: pick[t.id] })
                              }
                            >
